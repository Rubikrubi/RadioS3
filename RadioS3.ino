/*
 * ============================================================================
 *  RadioS3 v1.1 - autorskie radio internetowe na ESP32-S3-DEV-KIT-N16R8-M
 * ============================================================================
 ...
 *  cyklicznych. Wlasny kod (inspirowany dzialajacym projektem v11.1), nie kopia.
 *
 *  Funkcje (parytet):
...
void setup() {
  ...
  audio.setConnectionTimeout(1500, 5000);
...
void serviceAudio() {
  audio.loop();
  if (streaming()) vTaskDelay(1);   // oddaj czas WiFi/lwIP - inaczej glodowanie bufora (FLAC)
}
...
void handleEncoder() {
  ...
  if (menuOpen()) {
    ...
    if (ui.menuIntro) {
      ...
      ui.menuSel = ((ui.menuSel + step) % MENU_COUNT + MENU_COUNT) % MENU_COUNT;
    }
  ...
  switch (ui.mode) {
    ...
    case Mode::STATION:
      if (stationCount > 0) {
        ...
        sys.current = ((sys.current + d) % n + n) % n;
        sys.configDirty = true;
      }
      break;
    default:
...
void doConnect() {
  ...
  if (stationIsFlac(target)) Serial.println("Tryb FLAC: rzadsze WWW/OTA/OLED.");
...
void handleHealth() {
  ...
  if (sys.state == State::PLAY && running && sys.retry != 0 &&
      now - stats.sessionStart > 30000UL) {
    sys.retry = 0;
  }
...
void handleNet(unsigned long now) {
  ...
  bool s = streaming();
  if (!s || now - lastWeb >= NET_WEB_MS) {
    lastWeb = now;
    server.handleClient();
  }
  if (otaReady && (!s || now - lastOta >= NET_OTA_MS)) {
    lastOta = now;
    ArduinoOTA.handle();
  }
}
  ...
  do {
    ...
    if (menuOpen()) {
      ...
      if (ui.msg1[0]) {
        ...
        display.drawStr((128 - w) / 2, 25, ui.msg1);
      }
      if (ui.msg2[0]) {
        int w = display.getStrWidth(ui.msg2);
        display.drawStr((128 - w) / 2, 45, ui.msg2);
      }
    } else {
      if (ui.msgUntil != 0) ui.msgUntil = 0;
      switch (ui.mode) {
...
void drawBoot() {
  ...
  do {
    ...
    display.drawFrame(3, 3, 122, 58);
    display.setFont(u8g2_font_luBS10_tr);
    const char* t = "RadioS3";
    display.drawStr((128 - display.getStrWidth(t)) / 2, 25, t);
    display.setFont(u8g2_font_5x8_tr);
    char v[20];
    snprintf(v, sizeof(v), "v%s + OTA", FW_VERSION);
    display.drawStr((128 - display.getStrWidth(v)) / 2, 39, v);
    display.drawFrame(24, 48, 80, 6);
    display.drawBox(27, 50, 74, 2);
  } while (display.nextPage());
  delay(900);
}
...
void drawClock() {
  ...
  if (getLocalTime(&t, 10)) {
    ...
    display.drawStr((128 - w) / 2, 50, s);
  } else {
    display.setFont(u8g2_font_luBS10_tr);
    const char* m = "Oczekuje NTP...";
    display.drawStr((128 - display.getStrWidth(m)) / 2, 35, m);
  }
}
...
void drawWeather() {
  ...
  if (weather.loading && !weather.has) {
    ...
    display.drawStr((128 - display.getStrWidth(m)) / 2, 37, m);
    return;
  }
  if (!weather.has) {
...
void drawVolume() {
  ...
  display.drawStr((128 - display.getStrWidth(title)) / 2, 15, title);
  ...
  display.drawStr((128 - display.getStrWidth(v)) / 2, 55, v);
}
...
void drawStation() {
  if (!validIndex()) {
    ...
    display.drawStr((128 - display.getStrWidth(m)) / 2, 25, m);
    const char* m2 = "Dodaj przez WWW";
    display.drawStr((128 - display.getStrWidth(m2)) / 2, 45, m2);
    return;
  }
  display.setFont(u8g2_font_luBS10_tr);
  ...
  char shown[25];
  if (strlen(name) > 20) {
    strncpy(shown, name, 17);
    shown[17] = '\0';
    strcat(shown, "...");
  } else {
    strcpy(shown, name);
  }
  display.drawStr((128 - display.getStrWidth(shown)) / 2, 25, shown);

  char info[16];
  snprintf(info, sizeof(info), "%d/%d", sys.current + 1, (int)stationCount);
  display.drawStr((128 - display.getStrWidth(info)) / 2, 45, info);
  ...
  display.drawStr((128 - display.getStrWidth(status)) / 2, 60, status);
}
...
bool loadStore() {
  ...
  if (!f) {
    if (LittleFS.exists(CFG_BAK_PATH)) {
      ...
      if (LittleFS.rename(CFG_BAK_PATH, CFG_PATH)) return loadStore();
    }
    return false;
  }
  ...
  sys.eqMid = constrain(sys.eqMid, -12, 12);
  sys.eqHigh = constrain(sys.eqHigh, -12, 12);
...
void sysHealth() {
  ...
  if (timeReady && net.up && time(nullptr) < 1700000000L &&
      millis() - lastNtpKick > 300000UL) {
    lastNtpKick = millis();
    configTzTime(TZ_INFO, NTP_SERVER);
...
bool heapOk() {
  ...
  return heap > HEAP_MIN && freeFs > 1024;
}
